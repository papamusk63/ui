...
#-------------------------------------#
my $bed_status = "bed_status";
my $job_waiting = "job_waiting";
my $job_running = "job_running";
my $finish_time = "finish_time";
my $pause_info = "pause_info";
my $special_notes = "special_notes";
my $bed_dates = "bed_dates";
#-------------------------------------#
...
sub get_test_bed_list {
	...
	while (<STATUS>) {
		...
		next if (!$include_spr && $bed =~ /spr|dead/);
...
t detail>" : " ";
        my $job_waiting_string = "<a href=http://$host/new_UI/jobs/ title=Checkout detail>".$image
...
sub get_idle {
	...
	my $number_of_waiting = $testbed_waiting{$tb};
        my $s = $number_of_waiting gt 1? "s" : "";
        my $image = $number_of_waiting gt 0? "<img src=images/job_waiting.gif title=Checkout detail>" : " ";
        my $job_waiting_string = "<a href=http://$host/new_UI/jobs/ title=Checkout detail>".$image.$number_of_waiting." job$s waiting</a>";
